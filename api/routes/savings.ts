...
router.post("/:id/deposit", async (req, res) => {
  ...
  try {
    ...
    if (phone) {
      sendDepositSms(phone, (transaction.account as any).member.name, numAmount, (transaction.account as any).member.memberId, depositMonth, transactionDate, voucherNo)
...
router.get("/reports/expense-statement", async (req, res) => {
  ...
  try {
    ...
    const investmentCategoryMap: Record<string, { amount: number; count: number }> = {};
    let totalExpense = 0;
