...
export default function Members() {
  ...
  const [formData, setFormData] = useState({
    name: "", phone: "", email: "", nid: "", address: "", type: "General", position: "Member", joinDate: new Date().toISOString().split('T')[0]
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      fetchMembers();
      setFormData({ name: "", phone: "", email: "", nid: "", address: "", type: "General", position: "Member", joinDate: new Date().toISOString().split('T')[0] });
      setPhotoFile(null);
    } catch (error: any) {
      alert(error.response?.data?.message || "Error creating member");
    }
  };
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan={6} className="text-center py-8 text-slate-500">কোনো সদস্য পাওয়া যায়নি</td></tr>
                ...
                <tr key={member.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <button className="text-slate-400 hover:text-blue-600 transition"><Edit size={18} /></button>
                    <button className="text-slate-400 hover:text-red-600 transition"><Trash2 size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
            <div className="p-6 border-b flex justify-between items-center">
              <h3 className="text-xl font-bold text-slate-800">নতুন সদস্য ভর্তি</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">ছবি</label>
